...
ORIGINAL_DATASET_ID = 'networkrail'
DATASET_ID = 'dbt_e2e_networkrail'
TABLE_ID = ['movements']
N_SAMPLE = 100
DEFAULT_LLM_PROVIDER = 'OPENAI'
MAINTAIN_ORIGINAL_SCHEMA = False
MAINTAIN_ORIGINAL_TEST = False
PII_KEYWORDS = [
    "name",
    "first_name",
    "last_name",
...
def skip_dbt_test():
    raise AirflowSkipException("Temporarily skipping dbt test")
...
with DAG(
    dag_id=f"networkrail_metadata_pipeline",
    default_args=default_args,
    schedule=None,
    catchup=False,
    tags=["Metadata", "networkrail"],
):
    ...
    system_prompt = PythonOperator(
        ...
        op_kwargs={
            "path_to_yaml": f"{DBT_PROJECT_DIR}/models/staging/_models.yml",
            "maintain_original_schema": MAINTAIN_ORIGINAL_SCHEMA,
            "maintain_original_test": MAINTAIN_ORIGINAL_TEST,
        },
    )
    ...
    capture_tests_before = PythonOperator(
        ...
        op_kwargs={
            "path_to_yaml": f"{DBT_PROJECT_DIR}/models/staging/_models.ymll",
        },
    )
    ...
    write_schema = PythonOperator(
        ...
        op_kwargs={
            ...
            "path_to_save_yaml": f"{DBT_PROJECT_DIR}/models/staging/_models.yml",
            # "path_to_save_yaml": f"{DBT_PROJECT_DIR}/models/staging/networkrail/_{TABLE_ID}_schema.yml",
        },
    )
    ...
    capture_tests_after = PythonOperator(
        ...
        op_kwargs={
            # IMPORTANT: need to declare how to write the result to ensure, no schema duplicate
            "path_to_yaml": f"{DBT_PROJECT_DIR}/models/staging/_models.yml",
            # "path_to_yaml": f"{DBT_PROJECT_DIR}/models/staging/networkrail/_{TABLE_ID}_schema.yml",
        },
    )
